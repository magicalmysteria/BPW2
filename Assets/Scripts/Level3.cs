...
public class Level3 : MonoBehaviour
{
    ...
    Dictionary<Vector2, GameObject> gameTiles;
    List<Vector2Int> tilesAroundPlayer;
    [HideInInspector]
    public int currentPlayerNumber = 1;
    Player oppositePlayer;

    [HideInInspector]
    public round roundState = round.BOMB;
    ...
    private void PlayRound(Player currentPlayer)
    {
        ...
        else if (roundState == round.BOMB)
        {
            ThrowBomb(currentPlayer);
        }
    ...
    private void ResetRound()
    {
        ...
        // Begin next round
        if(player1.hasBomb == false && currentPlayerNumber == 1) roundState = round.BEGIN;
        if(player2.hasBomb == false && currentPlayerNumber == 2) roundState = round.BEGIN;
        if(player2.hasBomb == true && currentPlayerNumber == 2) roundState = round.BOMB;

    }

    // Throw a bomb at the beginning of the level
    private void ThrowBomb(Player currentPlayer)
    {
        if (Input.GetKeyDown(KeyCode.Mouse0))
        {
            ...
            for (int x = (int)hitX - 1; x < (int)hitX + 2; x++)
            {
                for (int y = (int)hitY - 1; y < (int)hitY + 2; y++)
                {
                    if (x >= 0 && x < fieldSize.x && y >= 0 && y < fieldSize.y)
                    {
                        gameTiles[new Vector2(x, y)].GetComponent<PlayingTile>().stepScore--;
                        if (gameTiles[new Vector2(x, y)].GetComponent<PlayingTile>().stepScore == 0)
                        {
                            gameTiles[new Vector2(x, y)].GetComponent<PlayingTile>().SetToZero(currentPlayer);
                        }
                        else if (gameTiles[new Vector2(x, y)].GetComponent<PlayingTile>().stepScore > 0)
                        {
                            gameTiles[new Vector2(x, y)].GetComponent<PlayingTile>().GetComponentsInChildren<TextMesh>()[0].text = gameTiles[new Vector2(x, y)].GetComponent<PlayingTile>().stepScore.ToString();
                        }
                    }
                }
            }
